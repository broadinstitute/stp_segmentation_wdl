...
def main(input_image, detected_transcripts, transform_mat, intervals, show):
    ...
    for i in range(0, len(result), interval_range):
        ...
        meta.to_csv(f"tile_metadata_{j}.csv")
        ...
        cv.imwrite(f"tiled_image_{j}.tiff", current_tile)
        current_tile_transc.to_csv(f"tiled_detected_transcript_{j}.csv")
        ...
        if show:
            ...
            plt.savefig(f"tiled_overlay_{j}.png")
            plt.close()
