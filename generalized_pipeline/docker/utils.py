...
def tile_coords(image_height, image_width,
    ...
    rem_height: Width of the bottom border tiles 
    rem_width: Height of the right border tiles 
    ...
    tile_boundaries_list = list()

    for y_min in range(0, image_height, tile_height):

        if y_min <= image_height - tile_height:
            y_max = y_min + tile_height
        else:
            y_max = y_min + rem_height

        for x_min in range(0, image_width , tile_width):

            if x_min <= image_width - tile_width:
                x_max = x_min + tile_width
            else:
                x_max = x_min + rem_width    

            out = [y_min, y_max, x_min, x_max]
            tile_boundaries_list.append(out)
