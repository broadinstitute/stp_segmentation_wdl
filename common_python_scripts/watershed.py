...
# ---------------------------
# Image loading (unchanged API)
# ---------------------------
def load_and_preprocess(image_path):
    """
    Preserves original behavior for image-like inputs,
    but uses tifffile when loading density TIFFs.
    """
    try:
    ...
    # Density images should already be 2D
    if image.ndim == 2:
        return image.astype(np.float32)

    # Original RGB / grayscale logic preserved
    if image.ndim == 3:
        if image.shape[2] == 3:
...
def generate_labels(gray):
    """
    Automatically adapts behavior depending on input type:
    - density image (Option A) → percentile thresholding + no double smoothing
    - image-like input → original Yen-based logic
    """

    # Heuristic: float images with large dynamic range are density fields
    is_density = (
        np.issubdtype(gray.dtype, np.floating)
        and np.percentile(gray, 99) > 1.0
    )

    if is_density:
        # ---- Option A path (new edits) ----
...
# ---------------------------
def extract_polygons_from_labels(labels, min_area=5):
    mask = np.zeros_like(labels, dtype=np.uint8)
    polygons = []
...
def main(image_paths):
    ...
    for image_path in image_paths:
        ...
        print(" shape:", gray.shape, gray.dtype)
        print(" n_labels:", int(labels.max()))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Watershed segmentation (image + density aware)')
